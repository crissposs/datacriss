...
async function spanishto(){
    CX.command('spanishto', async (ctx) => {
        logCommand(ctx);

        const commandArgs = ctx.message.text.split(' ');
        if (commandArgs.length >= 3) {
          const sourceLanguage = 'es'; // Idioma de origen fijo en español
          ...
          try {
            ...
            cx.success('Traduccion exitosa')

          } catch (error) {
            CX.error('Error al traducir el texto');
